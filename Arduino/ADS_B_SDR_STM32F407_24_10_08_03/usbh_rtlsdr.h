 ...
 *  Примечание о размерах буфера **
  ...
  * структура изменяется. Вероятно причина в stm32f4xx_ll_usb.c*
*/
...
* коэффициентами. Указаны только первые 16 коэффициентов, остальные 16
* используют те же значения, но в обратном порядке. Первый коэффициент в
* массиве — внешний, последний, последний — внутренний.
* Первые 8 коэффициентов — это 8-битные целые числа со знаком, следующие 8 коэффициентов
* — это 12-битные целые числа со знаком. Все коэффициенты имеют одинаковый вес.
*
* Коэффициенты FIR по умолчанию, используемые для DAB/FM драйвером Windows,
* драйвер DVB использует другие
 */
