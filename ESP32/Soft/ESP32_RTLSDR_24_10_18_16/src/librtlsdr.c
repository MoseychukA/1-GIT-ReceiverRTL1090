...
const char* TAG_LIBRTLSDR = "LIBRTLSDR";
...
int rtlsdr_set_sample_rate(rtlsdr_dev_t *dev, uint32_t samp_rate)
{
    ...
    fprintf(stderr, "** Start rtlsdr_set_sample_rate\n");
    int r = 0;
    uint16_t tmp;
...
int rtlsdr_read_sync(rtlsdr_dev_t *dev, void *buf, int len, int *n_read)
{
    if (!dev)
        return -1;
    fprintf(stderr, "****rtlsdr_dev_t *dev %d\n", dev->driver_obj);

    int read_sync = esp_libusb_bulk_transfer(dev->driver_obj, 0x81, buf, len, n_read, BULK_TIMEOUT);
    fprintf(stderr, "****read_sync =  %d\n", read_sync);

    return read_sync;//esp_libusb_bulk_transfer(dev->driver_obj, 0x81, buf, len, n_read, BULK_TIMEOUT);
}
