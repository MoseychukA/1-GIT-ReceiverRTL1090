...
const char* TAG_ADSB_LIB = "ADSB_LIB";
...
{
    ESP_LOGI(TAG_ADSB_LIB, "esp_libusb_bulk_transfer info %d %d %d %d", endpoint, length, timeout, *transferred);
    assert(driver_obj->dev_hdl != NULL);
    /*size_t*/int sizePacket = 256;//!! usb_round_up_to_mps(length, 64);
    usb_transfer_t *transfer = NULL;
    usb_host_transfer_alloc(sizePacket, 0, &transfer);
    ESP_LOGI(TAG_ADSB_LIB, "esp_libusb_bulk_transfer usb_host_transfer_alloc %d", sizePacket);
    ...
    ESP_LOGI(TAG_ADSB_LIB, "transfer->device_handle =  %d", driver_obj->dev_hdl);
    ESP_LOGI(TAG_ADSB_LIB, "transfer->bEndpointAddress =  %d", endpoint);
    ESP_LOGI(TAG_ADSB_LIB, "transfer->callback = %d", bulk_transfer_read_cb);
    ESP_LOGI(TAG_ADSB_LIB, "transfer->context = %d", (void*)&driver_obj);
    ESP_LOGI(TAG_ADSB_LIB, "transfer->timeout_ms = %d", timeout);
    ...
    */
    ...
    if (r != ESP_OK)
    {
        ESP_LOGI(TAG_ADSB_LIB, "esp_libusb_bulk_transfer failed with %d", r);
        return -1;
    }
    ...
    {
        ESP_LOGI(TAG_ADSB_LIB, "esp_libusb_bulk_transfer failed");
        return -1;
    }
...
int esp_libusb_control_transfer(class_driver_t *driver_obj, uint8_t bm_req_type, uint8_t b_request, uint16_t wValue, uint16_t wIndex, unsigned char *data, uint16_t wLength, unsigned int timeout)
{
    ...
    if (r != ESP_OK)
    {
        ESP_LOGI(TAG_ADSB_LIB, "libusb_control_transfer failed with %d", r);
        return -1;
    }
    ...
    if (!adsbdev->is_success)
    {
        ESP_LOGI(TAG_ADSB_LIB, "libusb_control_transfer failed");
        return -1;
    }
    for (uint8_t i = 0; i < wLength; i++)
    {
