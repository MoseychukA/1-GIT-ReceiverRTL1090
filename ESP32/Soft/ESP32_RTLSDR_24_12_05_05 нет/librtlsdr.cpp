...
int esp_libusb_bulk_transfer(class_driver_t* driver_obj, unsigned char endpoint, unsigned char* data, int length, int* transferred, unsigned int timeout)
{
    ...
    /**
* @brief Освобождение объекта передачи
*
* - Освобождение объекта передачи, ранее выделенного с помощью usb_host_transfer_alloc()
* - Передача не должна быть в процессе при попытке ее освобождения
* - Если передан указатель NULL, эта функция просто вернет ESP_OK
*
* @param[in] transfer Объект передачи
* @return esp_err_t
*/
    esp_err_t r = usb_host_transfer_free(transfer);
  ...
  */
    ...
    r = usb_host_transfer_submit(transfer); // Эта функция отправляет запрос на передачу данных и немедленно возвращает значение. Возвращает 0 в случае успеха
...
bool libusb_control_transfer(class_driver_t* driver_obj, uint8_t bm_req_type, uint8_t b_request, uint16_t wValue, uint16_t wIndex, unsigned char* data, uint16_t wLength, unsigned int timeout)
{
    ...
    if (r != ESP_OK)
    {
        ESP_LOGI(TAG_LIBRTLSDR, "libusb_control_transfer failed with %d", r);
        return false;
    }
...
int rtlsdr_read_array(rtlsdr_dev_t* dev, uint8_t block, uint16_t addr, uint8_t* array, uint8_t len)
{
    ...
    unsigned char* data = (unsigned char*)ps_calloc(len, sizeof(char));
    int ret = libusb_control_transfer(dev->driver_obj, CTRL_IN, 0, addr, index, data, len, CTRL_TIMEOUT);
    //fprintf(stderr, "rtlsdr_read_array here %d %d\n", ret, data[0]);
    *array = data[0];
    return ret;
}
...
int rtlsdr_write_array(rtlsdr_dev_t* dev, uint8_t block, uint16_t addr, uint8_t* array, uint8_t len)
{
    ...
    unsigned char* data = (unsigned char*)ps_calloc(len, sizeof(char));
    data[0] = *array;
    int ret = libusb_control_transfer(dev->driver_obj, CTRL_OUT, 0, addr, index, data, len, CTRL_TIMEOUT);
    //fprintf(stderr, "rtlsdr_write_array here %d\n", ret);
    free(data);
    return ret;
}
...
uint16_t rtlsdr_read_reg(rtlsdr_dev_t* dev, uint8_t block, uint16_t addr, uint8_t len)
{
    ...
    int r = libusb_control_transfer(dev->driver_obj, CTRL_IN, 0, addr, index, data, len, CTRL_TIMEOUT);
    fprintf(stderr, "rtlsdr_read_reg here %d\n", r);
    if (r < 0)
...
int rtlsdr_write_reg(rtlsdr_dev_t* dev, uint8_t block, uint16_t addr, uint16_t val, uint8_t len)
{
    ...
    int r = libusb_control_transfer(dev->driver_obj, CTRL_OUT, 0, addr, index, data, len, CTRL_TIMEOUT);
   // fprintf(stderr, "rtlsdr_write_reg here %d\n", r);
    if (r < 0)
        fprintf(stderr, "%s failed\n", __FUNCTION__);
...
uint16_t rtlsdr_demod_read_reg(rtlsdr_dev_t* dev, uint8_t page, uint16_t addr, uint8_t len)
{
    ...
    int r = libusb_control_transfer(dev->driver_obj, CTRL_IN, 0, addr, index, data, len, CTRL_TIMEOUT);
    //fprintf(stderr, "rtlsdr_demod_read_reg here %d\n", r);
    if (r < 0)
        fprintf(stderr, "%s failed\n", __FUNCTION__);
...
int rtlsdr_demod_write_reg(rtlsdr_dev_t* dev, uint8_t page, uint16_t addr, uint16_t val, uint8_t len)
{
    ...
    data[1] = val & 0xff;
    //fprintf(stderr, "rtlsdr_demod_write_reg here\n");
    int r = libusb_control_transfer(dev->driver_obj, CTRL_OUT, 0, addr, index, data, len, CTRL_TIMEOUT);
    if (r < 0)
        fprintf(stderr, "%s failed\n", __FUNCTION__);
...
{
    assert(dev->driver_obj->dev_hdl != NULL);
    fprintf(stderr, "Getting config descriptor\n");
...
{
  //  fprintf(stderr, "addr rtlsdr_read_sync = dev %d\n\n", dev); //Адрес устройства
    if (!dev)
        return -1;

    int esp_ret = esp_libusb_bulk_transfer(dev->driver_obj, 0x81, (unsigned char*)buf, len, n_read, BULK_TIMEOUT);
   // fprintf(stderr, "!!!!esp_ret: %d\n\n", esp_ret); // Результат.
    return esp_ret; //!! esp_libusb_bulk_transfer(dev->driver_obj, 0x81, (unsigned char*)buf, len, n_read, BULK_TIMEOUT);
}
