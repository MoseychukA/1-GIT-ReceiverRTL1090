...
SemaphoreHandle_t signaling_sem = xSemaphoreCreateBinary(); //  Создать бинарный семафор
...
void setup()
{
    ...
    vTaskDelay(10);    //Добавьте небольшую задержку, чтобы дать задачам выполниться
...
static void client_event_cb(const usb_host_client_event_msg_t* event_msg, void* arg)
{
    ...
    case USB_HOST_CLIENT_EVENT_NEW_DEV:
        ...
        if (driver_obj->dev_addr == 0)  // при старте адрес не определен
        {
            ...
            ESP_LOGI(TAG_DAEMON, "**driver_obj->client_hdl: %d", driver_obj->client_hdl);
            /*
            ...
            * @return esp_err_t
            */
            /* открыть модуль RTLSDR (свисток)*/
            rtlsdr_open(&rtldev, event_msg->new_dev.address, driver_obj->client_hdl);
            //esp_err_t err = usb_host_device_open(driver_obj->client_hdl, event_msg->new_dev.address, &driver_obj->dev_hdl);
           ...
           // esp_err_t err = usb_host_device_open(driver_obj->client_hdl, event_msg->new_dev.address, &driver_obj->dev_hdl);
            ...
            uint8_t* buffer = (uint8_t*)ps_malloc(out_block_size * sizeof(uint8_t)); // Определяем адрес хранения буфера
            ESP_LOGI(TAG_CLASS, "buffer: %d", buffer);
            int n_read = 2;
            ESP_LOGI(TAG_CLASS, "[APP] Free memory: %ld bytes", esp_get_free_heap_size());
           // ESP_LOGI(TAG_CLASS, "*** driver_obj->dev_hdl %d", driver_obj->dev_hdl);
            // uint32_t bytes_to_read = 0;
            while (true)
            {
                r = rtlsdr_read_sync(rtldev, buffer, out_block_size, &n_read);
                //ESP_LOGI(TAG_CLASS,"*** while r: %d", r);
                //ESP_LOGI(TAG_CLASS, "[APP] Free memory: %ld bytes", esp_get_free_heap_size());
                if (r < 0)
                {
             ...
             esp_action_get_dev_desc(rtldev); // Параметры USB устройства
 
             /*Open the device next*/
            // driver_obj->actions |= ACTION_OPEN_DEV;  // Открывать только после закрытия строки: rtlsdr_open(&rtldev, event_msg->new_dev.address, driver_obj->client_hdl);
...
static void action_get_str_desc(class_driver_t* driver_obj)
{
    ...
    driver_obj->actions &= ~ACTION_GET_STR_DESC;
    driver_obj->actions |= ACTION_OPEN_RTLSDR;
}

static void action_open_rtlsdr(class_driver_t* driver_obj)
{
    ESP_LOGI(TAG_CLASS, "Getting action_open_rtlsdr");
    /* открыть модуль RTLSDR (свисток)*/

    //driver_obj->dev_addr = event_msg->new_dev.address; // Определили адрес устройства
//            ESP_LOGI(TAG_DAEMON, "**driver_obj->dev_addr1: %d", driver_obj->dev_addr);
   // rtlsdr_open(&rtldev, 0x81, driver_obj->client_hdl);
...
void class_driver_task(void* arg)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (driver_obj.actions & ACTION_OPEN_RTLSDR)
            {
                action_open_rtlsdr(&driver_obj);
            }
