...
static const char* TAG_LIBRTLSDR = "LIBRTLSDR";
...
int rtlsdr_read_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
    ...
    // ESP_LOGI(TAG_LIBRTLSDR, "rtlsdr_read_array here %d %d", ret, data[0]);
    *array = data[0];
    return ret;
}
...
int rtlsdr_write_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
    ...
    // ESP_LOGI(TAG_LIBRTLSDR, "rtlsdr_write_array here %d", ret);
    free(data);
    return ret;
}
...
uint16_t rtlsdr_read_reg(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t len)
{
    ...
    ESP_LOGI(TAG_LIBRTLSDR, "rtlsdr_read_reg here %d", r);
    if (r < 0)
...
int rtlsdr_write_reg(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint16_t val, uint8_t len)
{
    ...
    ESP_LOGI(TAG_LIBRTLSDR, "rtlsdr_write_reg here %d", r);
    if (r < 0)
        fprintf(stderr, "%s failed\n", __FUNCTION__);
...
uint16_t rtlsdr_demod_read_reg(rtlsdr_dev_t *dev, uint8_t page, uint16_t addr, uint8_t len)
{
    ...
    int r = esp_libusb_control_transfer(dev->driver_obj, CTRL_IN, 0, addr, index, data, len, CTRL_TIMEOUT);
    // ESP_LOGI(TAG_LIBRTLSDR, "rtlsdr_demod_read_reg here %d", r);
    if (r < 0)
        fprintf(stderr, "%s failed\n", __FUNCTION__);
...
int rtlsdr_demod_write_reg(rtlsdr_dev_t *dev, uint8_t page, uint16_t addr, uint16_t val, uint8_t len)
{
    ...
    data[1] = val & 0xff;
    // ESP_LOGI(TAG_LIBRTLSDR, "rtlsdr_demod_write_reg here");
    int r = esp_libusb_control_transfer(dev->driver_obj, CTRL_OUT, 0, addr, index, data, len, CTRL_TIMEOUT);
    if (r < 0)
        fprintf(stderr, "%s failed\n", __FUNCTION__);
...
{
    int r = 0;
    ESP_LOGI(TAG_LIBRTLSDR, "*** dev->tuner1 %d", dev->tuner);
    ESP_LOGI(TAG_LIBRTLSDR, "*** dev %d", dev);
    if (!dev || !dev->tuner)
        return -1;
    ESP_LOGI(TAG_LIBRTLSDR, "*** dev->tuner2 %d", dev->tuner);
...
{
    assert(dev->driver_obj->dev_hdl != NULL);
    ESP_LOGI(TAG_LIBRTLSDR, "Getting config descriptor");
...
{
    ESP_LOGI(TAG_LIBRTLSDR, "*** rtlsdr_open0 %d", index);

    fprintf(stderr, "*** rtlsdr_open0 %d\n", index);
    int r;
    rtlsdr_dev_t *dev = NULL;
    uint8_t reg;
    ESP_LOGI(TAG_LIBRTLSDR, "*** rtlsdr_open %d", index);
    dev = malloc(sizeof(rtlsdr_dev_t));
    class_driver_t *driver_obj = calloc(1, sizeof(class_driver_t));
    if (NULL == dev)
        return -ENOMEM;
    ESP_LOGI(TAG_LIBRTLSDR, "*** client_hdl %d", client_hdl);
    ...
    ESP_LOGI(TAG_LIBRTLSDR, "rtl device number %d", reg);
...
int rtlsdr_read_sync(rtlsdr_dev_t *dev, void *buf, int len, int *n_read)
{
    if (!dev)
        return -1;
    fprintf(stderr, "*** rtlsdr_read_sync: %d \n", *n_read);
