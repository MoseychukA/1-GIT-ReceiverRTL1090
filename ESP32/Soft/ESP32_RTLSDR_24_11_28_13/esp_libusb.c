...
{
    printf("****!! esp_libusb_bulk_transfer info %X %d %d %d\n", endpoint, length, timeout, *transferred);
    ...
    printf("****!!esp_libusb_bulk_transfer usb_host_transfer_alloc %d\n", sizePacket);
    ...
    ESP_LOGI(TAG_ADSB, "transfer->num_bytes %d", transfer->num_bytes);
    ESP_LOGI(TAG_ADSB, "transfer->device_handle %d", transfer->device_handle);
    ESP_LOGI(TAG_ADSB, "transfer->bEndpointAddress %x", transfer->bEndpointAddress);
    ESP_LOGI(TAG_ADSB, "transfer->callback %d", transfer->callback);
    ESP_LOGI(TAG_ADSB, "transfer->context %d", transfer->context);
    ESP_LOGI(TAG_ADSB, "transfer->timeout_ms %d", transfer->timeout_ms);
    ...
    */
    ...
    if (r != ESP_OK)
    {
        printf("****!!esp_libusb_bulk_transfer failed with %d\n", r);
        return -1;
    }
    ...
    printf("****!!adsbdev->is_success %d\n", adsbdev->is_success);

    if (!adsbdev->is_success)
    {
        printf("##****!!esp_libusb_bulk_transfer failed\n");
